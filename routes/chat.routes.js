...
const tools = [
  ...
  },
  {
    type: "function",
    name: "search_books",
    description: "Busca libros en la base de datos por título, autor o género.",
    parameters: {
      type: "object",
      properties: {
        query: {
...
router.get("/history", isAuthenticated, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select("chatHistory chatMemory");
    ...
    return res.status(500).json({
      message: "Ha ocurrido un error al obtener el historial del chat",
    });
  }
});
...
router.post("/", isAuthenticated, async (req, res) => {
  try {
    ...
    if (lowerMsg.includes("cozy")) {
      updatedMemory.mood = "cozy";
    }

    if (lowerMsg.includes("romance")) {
      updatedMemory.preferredGenres = [
        ...new Set([...(updatedMemory.preferredGenres || []), "Romance"]),
      ];
    }

    if (lowerMsg.includes("no me gusta")) {
      updatedMemory.dislikedGenres = [
        ...new Set([...(updatedMemory.dislikedGenres || []), "Romance oscuro"]),
      ];
    }

    const memoryContext = `
Contexto del usuario:
- Mood: ${updatedMemory.mood || "desconocido"}
- Géneros favoritos: ${updatedMemory.preferredGenres?.join(", ") || "ninguno"}
- Géneros que no le gustan: ${updatedMemory.dislikedGenres?.join(", ") || "ninguno"}
- Prefiere respuestas cortas: ${updatedMemory.shortReplies ? "sí" : "no"}
`;
...
`.trim(),
    ...
    if (functionCalls.length === 0) {
      const assistantReply = cleanText(firstResponse.output_text);

      await User.findByIdAndUpdate(userId, {
        chatMemory: updatedMemory,
        $push: {
          chatHistory: {
            $each: [
    ...
    const assistantReply = cleanText(finalResponse.output_text);
